...
export class AllcardsTableComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.data$.pipe(filter<Allcards[]>(Boolean)).subscribe(allcardss => {
  ...
  createCustomer(allcards: Allcards): void {
    if (allcards) {
      ...
      if (isvalid) {
        const cardfullName = allcards.firstname.split(' ')
        let isdetault = true
          ...
          async resdata => {
            if (resdata.result != 'SAVE_FAILED' && resdata.result != 'DUPLICATE') {
              ...
              this.notification.error(
                'Error',
                'Failed to save card / Card details might already exists,Please try after sometime',
              )
            }
          },
          async error => {
  ...
  linkCard(transactionid, cardnumber) {
    ...
    this.apiProvider
      .postWithoutAuth('cardidentifications/checkversateccard', versateccontent)
      .subscribe(
        async versatecres => {
