...
export class CardbaseCurrencyComponent implements OnInit {
  ...
  ngOnInit() {
    this.onCardbasecurrencyForm = this.formBuilder.group({
      ...
      cardFullName: [null, Validators.compose([Validators.required, Validators.maxLength(30)])],
      fromAccount: [null, Validators.compose([Validators.required])],
      currency: [null, Validators.compose([Validators.required])],
      conversionRate: [null, Validators.compose([Validators.required])],
      amounttoCredit: [null, Validators.compose([Validators.required])],
      cardNumber: [
        null,
        Validators.compose([
          Validators.required,
  ...
  async getConversionAmount() {
    ...
    if (
      this.onCardbasecurrencyForm.value.fromAccount != null &&
  ...
  async getTransferedAmount() {
    const amounttocredit = (
      this.onCardbasecurrencyForm.value.fundAmount *
      this.onCardbasecurrencyForm.value.conversionRate
    ).toFixed(2)
  ...
  async latestUserDetails() {
    ...
    this.apiProvider
      .get('users/findbyMobile/' + mobileno + '/' + areacode + '/' + usertype)
      .subscribe(
        async resdata => {
        ...
        if (result.value) {
          ...
          this.apiProvider.get('configurations/comissiontype/CARD-WALLET').subscribe(
            async comdata => {
              ...
              const message =
                'You are about debit your card with ' +
                finalamount +
                ' ' +
                currencycode +
                ' to fund your wallet . Please confirm ?  '
              swal
                .fire({
                  title: message,
                  showCancelButton: true,
                      ...
                      async resdata => {
                        if (resdata.result == -1) {
                          ...
                          this.notification.warning(
                            'Warning',
                            'Failed to add fund from your card.Please try after sometime',
                          )
                        } else {
                          this.notification.success(
                            'Success',
                            'Amount has been added successfully to your wallet',
                          )
