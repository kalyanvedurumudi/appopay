...
export class RegisterComponent implements OnInit {
  ...
  enableStateDropdown() {
    const countryCode = this.validateForm.get('code').value;
    const filterdata = this.countries.filter(function (country) {
      return country.areacode == countryCode;
    });
  ...
  async validatePhone() {
    ...
    const areacode = this.validateForm.value.code;
  ...
  async goToPin() {
    ...
    const newnumber = '+' + this.validateForm.value.code + this.validateForm.value.mobileNumber;
    const verifydata = {
  ...
  createUser() {
    const usernames = this.validateForm.value.code + this.validateForm.value.mobileNumber;
    const insertdata = {
      ...
      transactionpin: this.validateForm.value.transactionPin,
      phonecode: this.validateForm.value.code,
      securityanswer: this.validateForm.value.achievment,
      screenlockpin: this.validateForm.value.mobilescreen,
      usertype: 'CUSTOMER',
      admincreate: 'N',
      role: [
        'USER'
      ...
      customerdetails: {
        ...
        cardtoken: null,
        countryid: this.validateForm.value.code,
        stateid: this.validateForm.value.state,
        address: this.validateForm.value.address,
        bankaccount: null,
        bankusername: '',
        customeraccount: [],
        cityname: this.validateForm.value.city,
        dob: this.validateForm.value.dob,
        currencyid: this.validateForm.value.currency,
        IdCuenta: null, // this.storage.retrieve('IdCuenta'),
        IdPlastico: null, // this.storage.retrieve('IdPlastico'),
        IdAsociado: null, // this.storage.retrieve('IdAsociado')
      }
    };
    this.spinner.show();
    this.apiProvider.postWithoutAuth('users', insertdata).subscribe(
      async rdata => {
        this.spinner.hide();
